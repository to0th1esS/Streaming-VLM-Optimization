...
def calc_average_metric(results, save_dir, metric, task, vmin=None, vmax=None):
    if isinstance(results, list):
        average_metric = sum([item[metric] for item in results if item['task']==task]) / len(results)
        ...
        for key, value in results.items():
            recalls = [item[metric] for item in value if item['task']==task]
            if len(value) > 0:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'{task}-{metric}.png'))
        plt.close()

        print(f'#Samples: {len(results[list(results.keys())[0]])}')
        print(average_recall)
        os.system(f"imgcat {os.path.join(save_dir, f'{task}-{metric}.png')}")
    else:
...
for task in ['plotQA', 'findNeedle', 'ego', 'count', 'order', 'anomaly_reco', 'topic_reasoning']:
    print(f'Task: {task}')
    calc_average_metric(results, args.save_dir, 'qa_acc', task)# vmin=0, vmax=100)

if 'pred_choice' in df.columns:
    n_errors = 0
    for _, row in df.iterrows():
        if row['pred_answer'] not in ['A', 'B', 'C', 'D', 'E']:
            n_errors += 1
            if args.debug:
