param(
    ...
    [string]$OutputDir = "results\turbovit_v1\v5_token_semantic",
    ...
    [double]$SparseRatioMin = 0.6,
    [double]$SparseRatioMax = 0.95,
    [int]$ProbeLayer = 2,
    [double]$SkipPatchThreshold = 0.001,
    [double]$DensePatchThreshold = 0.006,
    [double]$SkipFeatureThreshold = 0.9999,
...
& $MicromambaExe run -p $EnvPath python -m experiments.turbovit_v1.scripts.run_turbovit_v5 `
...
if ($LASTEXITCODE -ne 0) {
    throw "Turbo-ViT-v5 local run failed with exit code $LASTEXITCODE"
}
