param(
    ...
    [string]$OutputDir = "results\turbovit_v1\real_vit_dense",
    [string]$Weights = "none",
    [int]$NumFrames = 6,
    [int]$FrameStride = 4,
    [string]$Device = "auto"
...
& $MicromambaExe run -p $EnvPath python -m experiments.turbovit_v1.scripts.run_real_vit_dense `
...
if ($LASTEXITCODE -ne 0) {
    throw "Real ViT dense local run failed with exit code $LASTEXITCODE"
}
