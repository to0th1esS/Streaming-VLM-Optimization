...
from experiments.turbovit_v1.methods.turbovit_v5 import encode_stream_turbovit_v5
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Turbo-ViT-v5 token-level semantic reuse.")
    parser.add_argument("--output-dir", default="results/turbovit_v1/v5_token_semantic")
    ...
    parser.add_argument("--sparse-ratio-min", type=float, default=0.6)
    parser.add_argument("--sparse-ratio-max", type=float, default=0.95)
...
def main():
    ...
    v5_results = encode_stream_turbovit_v5(
        model,
        video,
    ...
    fidelity_rows = compare_outputs(dense_results, v5_results)
    decision_rows = summarize_decisions(v5_latency_rows)
    dense_total = sum(row["latency_ms"] for row in dense_latency_rows)
    v5_total = sum(row["latency_ms"] for row in v5_latency_rows)
    false_skip_count = sum(
        1
        for row, fidelity in zip(v5_latency_rows, fidelity_rows)
        if row["decision"] == "skip" and fidelity["output_cosine"] < 0.99
    )

    summary = {
        "experiment": "turbovit_v5_token_semantic_dual_anchor_reuse",
        "model": model_name,
        ...
        "v5_latency_ms_total": v5_total,
        "speedup": dense_total / v5_total if v5_total > 0 else 0.0,
    ...
    write_json(output_dir / "v5_summary.json", summary)
    write_csv(output_dir / "dense_latency.csv", dense_latency_rows)
    write_csv(output_dir / "v5_latency.csv", v5_latency_rows)
    write_csv(output_dir / "fidelity.csv", fidelity_rows)
    write_csv(output_dir / "decision_summary.csv", decision_rows)

    print("Turbo-ViT-v5 token semantic reuse completed")
    print(f"summary: {output_dir / 'v5_summary.json'}")
