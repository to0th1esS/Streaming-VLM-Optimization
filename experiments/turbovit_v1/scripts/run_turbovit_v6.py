...
from experiments.turbovit_v1.methods.turbovit_v6 import encode_stream_turbovit_v6
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Turbo-ViT-v6 token-group semantic reuse.")
    parser.add_argument("--output-dir", default="results/turbovit_v1/v6_token_group")
    ...
    parser.add_argument("--group-score", default="mean", choices=["mean", "min"])
        ...
        decision_rows.append(
            {
                ...
                "mean_dynamic_group_ratio": mean(subset, "dynamic_group_ratio"),
...
def main():
    ...
    v6_results = encode_stream_turbovit_v6(
        model,
        video,
    ...
    v6_latency_rows = [
        {
            ...
            "dynamic_group_ratio": result.dynamic_group_ratio,
        ...
        for result in v6_results
    ]
    fidelity_rows = compare_outputs(dense_results, v6_results)
    decision_rows = summarize_decisions(v6_latency_rows)
    dense_total = sum(row["latency_ms"] for row in dense_latency_rows)
    v6_total = sum(row["latency_ms"] for row in v6_latency_rows)
    false_skip_count = sum(
        1
        for row, fidelity in zip(v6_latency_rows, fidelity_rows)
        if row["decision"] == "skip" and fidelity["output_cosine"] < 0.99
    )

    summary = {
        "experiment": "turbovit_v6_token_group_semantic_reuse",
        "model": model_name,
        ...
        "v6_latency_ms_mean": mean(v6_latency_rows, "latency_ms"),
        "dense_latency_ms_total": dense_total,
        "v6_latency_ms_total": v6_total,
        "speedup": dense_total / v6_total if v6_total > 0 else 0.0,
    ...
    write_json(output_dir / "v6_summary.json", summary)
    write_csv(output_dir / "dense_latency.csv", dense_latency_rows)
    write_csv(output_dir / "v6_latency.csv", v6_latency_rows)
    write_csv(output_dir / "fidelity.csv", fidelity_rows)
    write_csv(output_dir / "decision_summary.csv", decision_rows)

    print("Turbo-ViT-v6 token-group semantic reuse completed")
    print(f"summary: {output_dir / 'v6_summary.json'}")
