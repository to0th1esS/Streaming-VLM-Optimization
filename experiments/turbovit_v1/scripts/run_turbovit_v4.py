...
from experiments.turbovit_v1.methods.turbovit_v4 import encode_stream_turbovit_v4
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Turbo-ViT-v4 semantic-stability adaptive reuse.")
    parser.add_argument("--output-dir", default="results/turbovit_v1/v4_semantic")
    ...
    parser.add_argument("--sparse-ratio-min", type=float, default=0.75)
    ...
    parser.add_argument("--skip-feature-threshold", type=float, default=0.9995)
...
def main():
    ...
    v4_results = encode_stream_turbovit_v4(
        model,
        video,
    ...
    v4_latency_rows = [
        {
            ...
            "semantic_to_rolling": result.semantic_to_rolling,
            "semantic_to_long": result.semantic_to_long,
        }
        for result in v4_results
    ]
    fidelity_rows = compare_outputs(dense_results, v4_results)
    decision_rows = summarize_decisions(v4_latency_rows)
    dense_total = sum(row["latency_ms"] for row in dense_latency_rows)
    v4_total = sum(row["latency_ms"] for row in v4_latency_rows)
    false_skip_count = sum(
        1
        for row, fidelity in zip(v4_latency_rows, fidelity_rows)
        if row["decision"] == "skip" and fidelity["output_cosine"] < 0.99
    )

    summary = {
        "experiment": "turbovit_v4_semantic_stability_adaptive_reuse",
        "model": model_name,
        ...
        "v4_latency_ms_mean": mean(v4_latency_rows, "latency_ms"),
        "dense_latency_ms_total": dense_total,
        "v4_latency_ms_total": v4_total,
        "speedup": dense_total / v4_total if v4_total > 0 else 0.0,
    ...
    write_json(output_dir / "v4_summary.json", summary)
    write_csv(output_dir / "dense_latency.csv", dense_latency_rows)
    write_csv(output_dir / "v4_latency.csv", v4_latency_rows)
    write_csv(output_dir / "fidelity.csv", fidelity_rows)
    write_csv(output_dir / "decision_summary.csv", decision_rows)

    print("Turbo-ViT-v4 semantic adaptive reuse completed")
    print(f"summary: {output_dir / 'v4_summary.json'}")
