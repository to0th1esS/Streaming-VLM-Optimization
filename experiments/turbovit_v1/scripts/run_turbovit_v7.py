...
def parse_args():
    parser = argparse.ArgumentParser(description="Run Turbo-ViT-v7 segment-aware semantic reuse.")
    parser.add_argument("--output-dir", default="results/turbovit_v1/v7_segment")
...
def main():
    ...
    decision_rows = summarize_decisions(v7_latency_rows)
    dense_total = sum(row["latency_ms"] for row in dense_latency_rows)
    v7_total = sum(row["latency_ms"] for row in v7_latency_rows)
    false_skip_count = sum(
        1
        for row, fidelity in zip(v7_latency_rows, fidelity_rows)
        if row["decision"] == "skip" and fidelity["output_cosine"] < 0.99
    )

    summary = {
        "experiment": "turbovit_v7_segment_aware_semantic_reuse",
        "model": model_name,
        ...
        "v7_latency_ms_mean": mean(v7_latency_rows, "latency_ms"),
    ...
    write_json(output_dir / "v7_summary.json", summary)
    write_csv(output_dir / "dense_latency.csv", dense_latency_rows)
    write_csv(output_dir / "v7_latency.csv", v7_latency_rows)
    write_csv(output_dir / "fidelity.csv", fidelity_rows)
    write_csv(output_dir / "decision_summary.csv", decision_rows)

    print("Turbo-ViT-v7 segment-aware semantic reuse completed")
    print(f"summary: {output_dir / 'v7_summary.json'}")
